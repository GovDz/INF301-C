...
/* Suppression des doublons consecutifs d'une sequence  : à implementer */
...
void suppression_doublons_consecutifs(liste_t *l) {
    cellule_t *cell, *suiv;
    cell = l->tete;
    while (cell != NULL && cell->suivant != NULL) {
        suiv = cell->suivant;
        if (cell->valeur == suiv->valeur) {
            cell->suivant = suiv->suivant;
            free(suiv);
        } else {
            cell = suiv;
        }
