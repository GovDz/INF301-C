...
def help():
    ...
    print("    or ", sys.argv[0],
          " py [t] (pour tester l'implementation Python)")
    print("    or ", sys.argv[0],
          " gen [t] (pour generer des fichiers de tests)")
    print()
    print(
        "Ou [t] peut etre laisse vide ou etre un parmi : base,long,nested,memfree,piiile")
    print("")
    print("ATTENTION : l'affichage dans vos programmes peut prendre beaucoup")
    print("de temps. Desactivez vos affichages ('print') en commentant les")
    print("lignes concernees, ou mieux en les mettant dans des blocs conditionnels:")
...
if prog and not os.path.exists(prog):
    ...
    print("Ce scrit est a lancer depuis le repertoire principal de travail de l'APP ainsi:")
...
time = subprocess.check_output(
    "which -a time | grep -v shell | tail -n 1", shell=True)
time = time.decode().rstrip()
# timecmd = " time -f '\tTemps: %es  Memoire max: %MKb' "
timecmd = time + " -f '%e;%M' "
timeout = 60
# timeout = 10
...
def run_test(mode, message):
    ...
    tmppipe = os.popen(
        "mktemp --tmpdir -u $USER.XXXXXXXXX.fifo").read().strip()
    ...
    while ret == None and size < size_max:
        ...
        if not gen:
            ...
            try:
                ...
                print("\tTemps: {}s  Memoire max: {:.2f} GB".format(
                    temps, float(memoire)))
...
all_tests = {
    "base": "(tres long sans blocs)",
    "long": "(tres long, beaucoup de blocs)",
    "nested": "(tres long, beaucoup de blocs imbriques)",
    "memfree": "(boucle executee de nombreuses fois)",
    "piiile": "(tres grande pile)",
}
